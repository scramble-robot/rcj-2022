...
void motorInit() {    // Arduinoのモーター制御を初期化する
  ...
  analogWriteFrequency(CH1PWM, 5000);
  analogWriteFrequency(CH2PWM, 5000);
  analogWriteFrequency(CH3PWM, 5000);
  analogWriteFrequency(CH4PWM, 5000);
...
void dribbler1(int power) { // ドリブラ１を回す. 0 < power <100
  if ( power < 0 || power > 100)
    return;
  if (power == 0) {
    digitalWrite(DR1PWM, 0);
  } else {
    digitalWrite(DR1PWM, HIGH);
  }
  return;
}
...
void dribbler2(int power) { // ドリブラ１を回す. 0 < power <100
  if ( power < 0 || power > 100)
    return;
  if (power == 0) {
    digitalWrite(DR2PWM, LOW);
  } else {
    digitalWrite(DR2PWM, HIGH);
  }
  return;
}
