...
from classes import WBsRGB as wb_srgb  # ホワイトバランスライブラリ
...
def white_balance_loops(img, h, w):
    ...
    result = wbModel.correctImage(img)  # white balance it

    return result
    # https://github.com/mahmoudnafifi/WB_sRGB/blob/master/WB_sRGB_Python/demo.py#L49
...
def main():
    ...
    #フレーム用意
    ret, frame = cap.read(
    )  #フレームの読み込み&次のフレームに移行 ret=bool(読み込めたか),frame=<class 'numpy.ndarray'>
    ...
    map_x = np.zeros(
        (h, w),
        dtype=np.float32)  #h*w次元配列を作る map_x,map_y=<class 'numpy.ndarray'>
    ...
    for i in range(h):  #Y座標
        for j in range(w):  #X座標
            ...
            Rp = (-(b2 - c2) * H * rp) / (
                (b2 + c2) * f -
                2 * b * c * math.sqrt(math.sqrt(pow(rp, 2) + f2)))
    ...
    while True:
        ...
        for i in range(Sep_num):
            for j in range(R_num):  #位置探し
                if (lower_yellow[0] <= frame[yT[i, j], xT[i, j], 0]
                        and frame[yT[i, j], xT[i, j], 0] <= upper_yellow[0]
                        and lower_yellow[1] <= frame[yT[i, j], xT[i, j], 1]
                        and frame[yT[i, j], xT[i, j], 1] <= upper_yellow[1]
                        and lower_yellow[2] <= frame[yT[i, j], xT[i, j], 2]
                        and frame[yT[i, j], xT[i, j], 2] <= upper_yellow[2]):
                    search[i] = True
            if search[i]:
        ...
        cv.line(frame, (Hw, Hh), (xT[0, 1], yT[0, 1]), (0, 255, 255),
                thickness=1,
                lineType=cv.LINE_AA)
